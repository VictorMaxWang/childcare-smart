# 2026-07-04 开放注册真实数据库版 T0 规则

本节优先级高于旧阶段描述，用于约束下一阶段“手机号 + 密码 + 确认密码注册，注册后直接写入真实数据库并创建真实机构/家庭空间”的开发边界。T0 只允许更新项目规则和开发规划文档；业务代码、页面、SQL 迁移和生产数据结构变更留到 T1-T9 执行。

## 项目定位与核心主链

- 慧育童行是面向托育场景的多角色、多智能体、移动端优先 Web 系统。
- 核心主链固定为：教师记录 -> 系统理解 -> 风险会诊 -> 园长承接 -> 家长执行 -> 反馈回流 -> 下一轮判断。
- 当前开发重点是开放注册真实数据库版，不是重做演示账号、视觉稿或三端主页面。
- 下一阶段所有改动都必须保护现有 teacher/admin/parent 主链入口和角色跳转逻辑。

## 认证策略

- 注册方式固定为手机号 + 密码；注册时必须输入两遍密码。
- 服务端必须校验 `confirmPassword`，不能只依赖前端校验。
- 注册成功后继续使用现有 HMAC `ccs_session` cookie 建立登录态。
- 登录页后续改为手机号 + 密码，但必须兼容旧 `username` 普通账号登录。
- 不引入第三方 SaaS 鉴权，不重写整个认证体系，不绕过现有 session cookie。

## 数据策略

- 注册后直接写入真实数据库，不使用邀请码，不默认进入 demo。
- 普通账号必须写入 `app_users` 且 `is_demo=false`，并创建真实 `institution_id` 对应的初始 `app_state_snapshots`。
- 下一阶段建议新增 `phone_normalized` 唯一字段；保留 `username_normalized` 兼容旧账号。
- 手机号标准化建议接受 `13800000000`、`+8613800000000`、`86 13800000000`，统一存储为 `+86` E.164 形式。
- 不要把真实用户写进 demo 账户、demo fixture、mock provider 或 demo snapshot。

## 数据隔离底线

- 任何儿童、班级、反馈、绘本、会诊、周报相关查询都必须绑定 `institution_id` 或 `child_ids`。
- 家长只能访问 session `childIds` 内的儿童，或明确绑定 `parentUserId` 的儿童。
- 教师只能访问同机构且同班级儿童；园长只能访问同机构数据，不新增跨机构 super-admin 行为。
- 不允许为了开放注册放宽 `scopeSnapshotForSessionUser`、`requireChildAccess`、`requireClassAccess`、AI route guard 或 API scope 规则。

## 儿童信息与监护人同意底线

- 家长创建儿童档案前必须有监护人确认与同意记录。
- 第一版儿童档案只收最小必要字段：姓名、出生日期、性别、监护人关系和必要联系电话；身高、体重、过敏、特殊关注项默认可后补。
- 监护人同意记录必须可审计，至少能表达同意时间、同意人、同意用途和账号/儿童绑定关系。
- 注册本身不应偷偷绕过儿童建档同意；家长空间可以先创建为空，再在 T7 完成合规建档。

## 禁止事项

- 不要引入第三方 SaaS 鉴权。
- 不要重写整个认证体系。
- 不要绕过现有 `ccs_session` HMAC session cookie。
- 不要把真实用户写进 demo 账户。
- 不要破坏现有 teacher/admin/parent 主链。
- 不要把验证码演示控件写成真实短信成功。
- 不要创建数据库迁移或改页面，除非当前任务明确进入对应 T 阶段。
