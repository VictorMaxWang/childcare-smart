...
function buildStorybookMediaStateKey(story: ParentStoryBookResponse | null) {
  if (!story) return "";
  return JSON.stringify({
    imageDelivery: story.providerMeta.imageDelivery,
    audioDelivery: story.providerMeta.audioDelivery,
    scenes: story.scenes.map((scene) => ({
      sceneIndex: scene.sceneIndex,
      imageStatus: scene.imageStatus,
      imageUrl: scene.imageUrl,
      assetRef: scene.assetRef,
...
export type StoryBookWorkspaceRole = "parent" | "teacher";

export default function StoryBookWorkspace({
  workspaceRole = "parent",
}: {
  workspaceRole?: StoryBookWorkspaceRole;
}) {
  ...
  const explicitDemoSeedId =
    workspaceRole === "parent"
      ? resolveParentStoryBookDemoSeedId(searchParams.get("demoSeed"))
      : null;
  ...
  const activeStoryRequestControllerRef = useRef<AbortController | null>(null);
  ...
  const isDemoStorybookAccount =
    workspaceRole === "parent" &&
    parentD01.currentUser.accountKind === "demo";
  ...
  const storybookSourceSummary = useMemo(() => {
    if (!selectedFeed) return [];
    const childId = selectedFeed.child.id;
    return [
      { label: "晨检", value: healthCheckRecords.filter((item) => item.childId === childId).length },
      { label: "饮食", value: mealRecords.filter((item) => item.childId === childId).length },
      { label: "成长", value: growthRecords.filter((item) => item.childId === childId).length },
      { label: "家园反馈", value: guardianFeedbacks.filter((item) => item.childId === childId).length },
      { label: "任务打卡", value: taskCheckInRecords.filter((item) => item.childId === childId).length },
      {
        label: "专业支持",
        value:
          Number(Boolean(getChildInterventionCard(childId))) +
          Number(Boolean(getLatestConsultationForChild(childId))),
      },
    ];
  }, [
    getChildInterventionCard,
  ...
  useEffect(() => {
    ...
    if (previousSelectedChildIdRef.current !== currentChildId) {
      previousSelectedChildIdRef.current = currentChildId;
      // 切换幼儿必须先断开上一名幼儿的正文和媒体请求，避免姓名已更新但内容仍短暂串档。
      activeStoryRequestControllerRef.current?.abort();
      activeStoryRequestControllerRef.current = null;
      storyRef.current = null;
      requestRef.current = null;
      lastStoryLoadKeyRef.current = null;
      pollingStoryIdRef.current = null;
      pollAttemptRef.current = 0;
      setStory(null);
      setStatus(workspaceRole === "teacher" ? "empty" : "loading");
  ...
  }, [selectedFeed?.child.id, workspaceRole]);
  ...
  const themeHint = parentD01.invalidChildId
    ? `当前${workspaceRole === "teacher" ? "教师" : "家长"}账号没有该 childId 的授权，系统不会自动回退到其他孩子。`
    : resolvedDemoSeedId
      ? "当前为本地演示生成，生成结果会保存到 D01 本地演示持久化。"
      : workspaceRole === "teacher" && !hasChildContext
        ? "当前教师账号没有可见幼儿，请先加入班级或确认班级已有幼儿档案。"
    ...
    const baseRequest = buildParentStoryBookRequestFromFeed({
      ...
      requestSource:
        workspaceRole === "teacher"
          ? "teacher-storybook-page"
          : "parent-storybook-page",
  ...
  useEffect(() => {
    ...
    activeStoryRequestControllerRef.current = controller;
    ...
    if (!backgroundMediaPoll && !bypassCache && !forceNetworkForManualOverride) {
      ...
      const cached =
        workspaceRole === "teacher"
          ? null
          : readParentStoryBookCache(resolvedCacheKey);
    ...
    if (
      ...
      startTransition(() => {
        ...
        setRefreshMessage(
          selectedFeed
            ? "选择参数后点击生成，系统不会在打开页面或切换幼儿时自动调用 AI。"
            : null
        );
        setIsRefreshing(false);
        setCacheState({ kind: "none" });
      ...
      return () => {
        cancelled = true;
        controller.abort();
        if (activeStoryRequestControllerRef.current === controller) {
          activeStoryRequestControllerRef.current = null;
        }
      };
    }
    ...
    async function loadStory() {
      ...
      try {
        ...
        if (cancelled) return;
        const previousMediaStateKey = buildStorybookMediaStateKey(storyRef.current);
        if (backgroundMediaPoll) {
          const currentStory = storyRef.current;
          if (
            !currentStory ||
        ...
        const mediaStateChanged =
          previousMediaStateKey !== buildStorybookMediaStateKey(data);
        const shouldPersistGeneratedStorybook =
          !forceNetworkForManualOverride &&
          (!backgroundMediaPoll || mediaStateChanged);
    ...
    return () => {
      ...
      if (activeStoryRequestControllerRef.current === controller && !keepUserGenerationRequestAlive) {
        activeStoryRequestControllerRef.current = null;
      }
    };
  }, [
  ...
  function handleSelectedChildChange(childId: string) {
    if (!parentD01.authorizedChildIds.has(childId)) return;

    const nextParams = new URLSearchParams(searchParams.toString());
    nextParams.set("child", childId);
    nextParams.delete("demoSeed");
    startTransition(() => {
    ...
    <StoryBookViewer
      ...
      parentHref={
        workspaceRole === "teacher"
          ? "/teacher"
          : selectedFeed?.child.id
            ? `/parent?child=${selectedFeed.child.id}`
            : "/parent"
      }
      tonightActionHref={
        workspaceRole === "teacher"
          ? selectedFeed?.child.id
            ? `/teacher/agent?scope=child&childId=${selectedFeed.child.id}`
            : "/teacher/agent"
          : selectedFeed?.child.id
            ? `/parent/agent?child=${selectedFeed.child.id}`
            : "/parent/agent"
      }
      feedbackHref={
        workspaceRole === "teacher"
          ? selectedFeed?.child.id
            ? `/teacher/agent?action=communication&childId=${selectedFeed.child.id}`
            : "/teacher/agent?action=communication"
          : selectedFeed?.child.id
            ? `/parent/agent?child=${selectedFeed.child.id}#feedback`
            : "/parent/agent#feedback"
      }
      workspaceRole={workspaceRole}
      availableChildren={parentD01.authorizedChildren}
      selectedChildId={selectedFeed?.child.id}
      sourceSummary={storybookSourceSummary}
      savedStorybookCount={savedStorybooks.length}
      onSelectedChildChange={
        workspaceRole === "teacher" ? handleSelectedChildChange : undefined
      }
