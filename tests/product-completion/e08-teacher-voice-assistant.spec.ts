...
test.describe("E08 teacher voice assistant skills", () => {
  test("teacher voice commands persist through E01 API and are parent/director visible", async ({}, testInfo) => {
    ...
    try {
      const morning = await planVoiceCommand(teacher, `给赵一诺记录晨检，体温三十六点八，状态正常 ${token}`);
      expect(morning.intent).toBe("create_morning_check");
      expect(morning.status).toBe("needs_confirmation");
      await executeVoiceCommand(teacher, `给赵一诺记录晨检，体温三十六点八，状态正常 ${token}`);
      ...
      await executeVoiceCommand(teacher, `记录赵一诺午餐吃完了 ${token}`);
      ...
      await executeVoiceCommand(teacher, `给赵一诺新增成长记录，今天会自己穿鞋 ${token}`);
      ...
      await executeVoiceCommand(teacher, `回复林妈妈，今天赵一诺午睡很好 ${token}`);
      ...
      await executeVoiceCommand(teacher, `给赵一诺创建健康材料解析任务 ${token}`);
      ...
      const consultationResult = await executeVoiceCommand(teacher, `给赵一诺创建高风险会诊 ${token}`);
      ...
      expect(directorConsultations.some((item: { consultationId?: string; workflowStatus?: string }) =>
        item.consultationId === consultationId && item.workflowStatus === "pending"
      )).toBe(true);

      await expectOk(
        await director.patch(`/api/consultations/${consultationId}/status`, {
  ...
  test("cross-class teacher voice command is forbidden and does not write", async ({}, testInfo) => {
    ...
    try {
      const command = await planVoiceCommand(teacher, `给赵一诺记录晨检，体温三十六点八，状态正常 ${token}`);
      ...
      const records = await expectOk(await teacher2.get(`/api/records?type=health&childId=${CHILD_ID}&includeArchived=1`));
  ...
  test("teacher uses VoiceOrb text fallback for record and navigation", async ({ page }) => {
    ...
    await page.getByTestId("voice-orb-input").fill(`给赵一诺记录晨检，体温三十六点八，状态正常 ${token}`);
