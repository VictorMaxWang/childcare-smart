...
| `app/api/ai/high-risk-consultation/feed/route.ts` | `authorizeAiRouteSession(request, { requiredRole: "staff", allowUnscoped: true })` 后加载服务端 session scope | feed 不接收客户端资源 scope；normal account 只从 session 可见 children/consultations 生成，指定 `child_id` 时再校验 child scope | brain 不可用或空结果时按当前 session scope 本地回退；真实账号无可见 child 时返回 `423 scope_required` | low | 保持 feed 上下文完全由服务端 session 构建，不允许客户端覆盖机构、班级或幼儿范围。 |
