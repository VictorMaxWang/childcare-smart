...
- 已完成：
  ...
  - 本地 `lint`、`typecheck`、production build、544 项 Node、227 项 Python 和 83 项发布浏览器回归已执行；浏览器结果为 80 通过、3 项真实账号规格明确跳过，因此只属于本地验证。
...
- 生产待完成：
  - 推送媒体超时补丁、确认 Vercel 提交 SHA，并重新完成 fresh smoke 的真实绘本图片/语音闭环。
